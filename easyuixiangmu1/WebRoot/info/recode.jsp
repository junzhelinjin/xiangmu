    ...
    <body>
       <header>
        ...
        <div class="weui_search_bar" id="search_bar">
              <form class="weui_search_outer" action="../info/recode" method="post">
           ...
           <c:forEach var="r" items="${requestScope.infolist3}">
                <a class="weui_cell" href="javascript:;">
                    <div class="weui_cell_bd weui_cell_primary">
                        ...
                        <div class="org">
                            ...
                            <label class="red">${r.istatusname}</label>
                            </c:if>
                            <c:if test="${r.status==6}">
                            <label class="gray">${r.istatusname}</label>
                            </c:if>
                            <c:if test="${r.status==7}">
                            <label class="yellow">${r.istatusname}</label>
                            </c:if>
                            <c:if test="${r.status==8}">
                            <label class="gray">${r.istatusname}</label>
                            </c:if>
                        </div>
                    </div>
